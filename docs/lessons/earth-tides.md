...
The existing Earth, Moon, Sun, sky, camera and simulation clock remain in place. Pan/zoom and time controls work normally. Explore/Orrery switching preserves the layer; leaving Earth, entering Sky or starting Artemis removes it. Turning it off removes the overlay without borrowing or restoring the clock or camera. The toolbar button exposes its active state with aria-pressed. A small noninteractive note retains the model qualification: “Water shape exaggerated. This simplified model leaves out coastlines and ocean depth.”
...
Orrery lunar directions use the same J2000 ecliptic frame as the planets. The compact orbit distance is 0.48 scene units; Moon size preserves the physical Moon/Earth diameter ratio. Explore spacing is unchanged.
