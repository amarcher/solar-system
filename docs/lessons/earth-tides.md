...
The water shell uses the degree-two potential `P2(cos θ) = (3 cos² θ − 1)/2`, summed with the same weights. Its displayed radius is `1.18 + 0.22 × potential`. The offset and displacement are artistic scale parameters, not physical ocean depths. The shell stays outside the Earth mesh for all supported source/phase combinations. This deliberately idealized global ocean has antipodal bulges and no coasts, ocean dynamics, terrain, or forecast quantities.
