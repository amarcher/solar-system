...
export function useTidesRecording(state: TidesState | null, update: (patch: Partial<TidesState>) => void) {
  ...
  const releaseClock = useRef<(() => void) | null>(null);
  ...
  const latest = useRef({ state, update, astronomy });
  useLayoutEffect(() => { latest.current = { state, update, astronomy }; }, [state, update, astronomy]);
  ...
  const restore = useCallback((saved: TidesState) => {
    ...
    releaseClock.current?.();
    releaseClock.current = null;
    if (mounted.current) {
  ...
  const start = useCallback(() => {
    ...
    releaseClock.current = borrowCaptureClock(latest.current.astronomy);
    frame.current = null;
  ...
  const onFrame = useCallback((next: TidesCaptureFrame) => {
    ...
    const recording = startTidesRecording({
      ...
      backgroundCredits: latest.current.astronomy.mode === 'artistic'
        ? ['Sky: Solar System Scope · CC BY 4.0 (source above)']
        : ['Sky: NASA/GSFC Scientific Visualization Studio · Gaia DR2: ESA/Gaia/DPAC',
          'svs.gsfc.nasa.gov/4851 · display-enhanced sky; sizes and distances compressed'],
