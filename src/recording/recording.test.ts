...
import { tidesCaption } from '../lessons/tides/model';
...
function setup() {
  ...
  const options: RecordingOptions = {
    ...
    backgroundCredits: ['Sky: NASA/GSFC Scientific Visualization Studio', 'Gaia DR2: ESA/Gaia/DPAC'],
...
describe('tides presentation', () => {
  it('uses three six-second live-scene beats without moving the Moon to phase presets', () => {
    expect([0, 6000, 12000].map(time => presentationAt(time).state)).toEqual([
      { step: 'gravity', source: 'moon', phase: 0, live: true },
      { step: 'difference', source: 'moon', phase: 0, live: true },
      { step: 'water', source: 'both', phase: 0, live: true },
    ]);
    expect(presentationAt(5999).chapter).toBe(0);
    expect(presentationAt(11999).chapter).toBe(1);
    expect(presentationAt(-100).chapter).toBe(0);
    expect(presentationAt(NaN).chapter).toBe(0);
    expect(presentationAt(RECORDING_DURATION_MS).chapter).toBe(2);
...
describe('live captions', () => {
  it('describes the selected source without claiming a scripted phase', () => {
    for (const source of ['moon', 'sun', 'both'] as const) {
      const caption = tidesCaption({ step: 'water', source, phase: 90, live: true });
      expect(caption.explanation).not.toMatch(/new|full|quarter|neap|spring/i);
      expect(caption.explanation).toContain(source === 'both' ? 'Moon and Sun' : source === 'moon' ? 'The Moon' : 'The Sun');
    }
  });
});
...
describe('actual canvas recorder lifecycle', () => {
  it('copies only a rendered matching state after rendering, with visible qualification and credits', async () => {
    ...
    for (const credit of env.options.backgroundCredits!) expect(text).toContain(credit);
    env.frame(50);
  ...
  it('never paints a new chapter over stale lesson pixels', async () => {
    ...
    env.frame(6000);
    expect(env.context.drawImage).toHaveBeenCalledTimes(1);
    env.setRendered(presentationAt(6000).state);
    env.frame(6040);
