...
## Browser acceptance, 2026-09-21

A real foreground run on the integration build produced H.264 MP4 at 720×1280, 18.024 seconds, 3,386,047 bytes, and 463 encoded frames (variable cadence, about 25.7 fps average; 30 fps is the capture request, not a guaranteed encoder rate). Frames from all six chapters were inspected, including full-Moon reinforcement and quarter-Moon reduction. The model qualification and image credits are present. Native in-app video playback reached the end without an error.

Cancellation and preview dismissal returned focus to Record. A manually orbited, paused Earth view survived closing the whole lesson during active recording after the camera restoration guard was corrected. The prior clock and Why tides focus were restored. Physical iPhone Safari remains a separate pending user check. No clip was uploaded or posted.
