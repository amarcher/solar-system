...
export function TidesRecordingControls({ recording, recordButtonRef }: { recording: TidesRecordingUi; recordButtonRef: RefObject<HTMLButtonElement | null> }) {
  ...
  return <div className={`tides-recording${recording.active ? ' tides-recording--active' : ''}`}>
    {recording.active
      ...
      : <button ref={recordButtonRef} type="button" disabled={!recording.available} aria-disabled={!recording.available || recording.status === 'finishing'} onClick={() => { if (recording.status !== 'finishing') recording.start(); }}>Record 18-second clip</button>}
