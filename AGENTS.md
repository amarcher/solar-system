...
- `realisticScale.ts` — log-compressed AU-to-scene-unit mapping for orrery mode. Use `scaleAUVector()` for positions so compression is radial from the Sun; reserve scalar `scaleAU()` for distances/radii.
...
- Orrery mode uses log-compressed AU (`realisticScale.ts`) to keep all planets visible. Position vectors must be compressed radially with `scaleAUVector()`; applying `scaleAU()` to each axis independently warps circular/elliptical paths into squarish orbits at high time acceleration.
